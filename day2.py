...
The elves are running low on wrapping paper, and so they need to submit an order for more. They have a list of the dimensions (length length, width width, and height height) of each present, and only want to order exactly as much as they need.

Fortunately, every present is a box (a perfect right rectangular prism), which makes calculating the required wrapping paper for each gift a little easier: find the surface area of the box, which is 2*length*width + 2*width*height + 2*height*length. The elves also need a little extra paper for each present: the area of the smallest side.
...
print(total_paper_needed(data))
print(total_ribbon_needed(data))


# To refactor :
# - factorize total_ribbon
# - hide responses variables in answer.txt
